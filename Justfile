...
BENCH_PACKAGES := '
    ca-certificates openssl@3 xz sqlite readline icu4c@78 python@3.14
    awscli node harfbuzz ncurses gh pcre2 libpng zstd glib lz4
    gettext libngtcp2 libnghttp3 pkgconf libunistring mpdecimal brotli
    jpeg-turbo xorgproto ffmpeg cmake libnghttp2 go uv gmp libtiff
    fontconfig python@3.13 git little-cms2 dav1d openexr c-ares tesseract
    p11-kit imagemagick zlib libx11 freetype protobuf gnupg openjph
    libtasn1 ruby gnutls expat libsodium simdjson gemini-cli libarchive
    pyenv pixman curl opus unbound cairo pango leptonica libxcb
    jpeg-xl coreutils certifi krb5 docker libheif webp libxext libxau
    gcc bzip2 libxdmcp abseil xcbeautify libuv giflib utf8proc libxrender
    m4 graphite2 openjdk uvwasi libffi libdeflate llvm aom lzo
    libevent libgpg-error libidn2 berkeley-db@5 deno libedit oniguruma
'
...
bench *args:
    ...
    # Validate sudo credentials once before starting benchmarks
    sudo -v
    ...
    for i in "${!PACKAGES[@]}"; do
        ...
        if BREW_MS=$(run_timed_install "Homebrew" brew install "$pkg"); then
            echo -e "    ${GREEN}OK: ${BREW_MS}ms${NORMAL}" >&2
        else
        ...
        if ZB_COLD_MS=$(run_timed_install "Zerobrew (cold)" zb install "$pkg"); then
            echo -e "    ${GREEN}OK: ${ZB_COLD_MS}ms${NORMAL}" >&2
        else
            echo -e "    ${RED}FAILED${NORMAL}" >&2
            FAILED_NAMES+=("$pkg")
        ...
        if ZB_WARM_MS=$(run_timed_install "Zerobrew (warm)" zb install "$pkg"); then
            echo -e "    ${GREEN}OK: ${ZB_WARM_MS}ms${NORMAL}" >&2
        else
            echo -e "    ${RED}FAILED${NORMAL}" >&2
            FAILED_NAMES+=("$pkg")
    ...
    output_text() {
        ...
        echo ""
        echo "Results:"
        echo "Package         Homebrew       ZB Cold        ZB Warm        Speedup (cold/warm)"
        echo "--------------------------------------------------------------------------------"
        for i in "${!NAMES[@]}"; do
            printf "%-15s %9s ms %10s ms %10s ms %10sx / %sx\n" "${NAMES[i]}" "${BREW_TIMES[i]}" "${ZB_COLD_TIMES[i]}" "${ZB_WARM_TIMES[i]}" "${SPEEDUPS_COLD[i]}" "${SPEEDUPS_WARM[i]}"
        done
        echo
        if [[ $FAILED -gt 0 ]]; then
    ...
    output_html() {
        ...
        echo '                <tr>'
        echo '                    <th>Package</th>'
        echo '                    <th>Homebrew (ms)</th>'
        echo '                    <th>ZB Cold (ms)</th>'
        echo '                    <th>ZB Warm (ms)</th>'
        ...
        for i in "${!NAMES[@]}"; do
            echo "                <tr>"
            echo "                    <td>${NAMES[$i]}</td>"
            echo "                    <td>${BREW_TIMES[$i]}</td>"
            echo "                    <td>${ZB_COLD_TIMES[$i]}</td>"
            echo "                    <td>${ZB_WARM_TIMES[$i]}</td>"
