...
pub async fn execute(
    ...
    let progress_callback: Arc<ProgressCallback> = Arc::new(Box::new(move |event| {
        // Minimize mutex hold time: clone the ProgressBar (which is Arc-based
        // internally) and drop the lock before calling any methods on it.
        // This prevents contention when multiple chunked-download tasks fire
        // progress events concurrently, which was causing the bar to freeze.
        match event {
            InstallProgress::DownloadStarted { name, total_bytes } => {
                let pb = if let Some(total) = total_bytes {
                    ...
                    // Enable a steady tick so the bar redraws periodically
                    // even when set_position calls are rate-limited internally.
